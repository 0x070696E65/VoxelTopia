...
public class Clouds : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        LoadCloudData();
        CreateClouds();
        world.OnStartCheckViewDistance += UpdateClouds;
        UpdateClouds();
    }
    ...
    private void LoadCloudData()
    {
        ...
        var cloudTex = cloudpattern.GetPixels();
        
        // Loop through colour array and set bools depending on opacity of clolour.
        for (var x = 0; x < cloudTexWidth; x++)
        {
            for (var y = 0; y < cloudTexWidth; y++)
            {
    ...
    private void CreateClouds()
    {
        ...
        for (var x = 0; x < cloudTexWidth; x += cloudTileSize) {
            for (var y = 0; y < cloudTexWidth; y += cloudTileSize)
            {
                var cloudMesh = world.settings.clouds == CloudStyle.Fast ? CreateFastCloudMesh(x, y) : CreateFancyCloudMesh(x, y);
    ...
    public void UpdateClouds()
    {
        ...
        for (var x = 0; x < cloudTexWidth; x += cloudTileSize)
        {
            for (var y = 0; y < cloudTexWidth; y += cloudTileSize)
            {
                ...
                position = new Vector3(RoundCloud(position.x), cloudHeight, RoundCloud(position.z));
    ...
    private int RoundCloud(float value)
    {
    ...
    private Mesh CreateFastCloudMesh(int x, int z)
    {
        ...
        for (var xIncrement = 0; xIncrement < cloudTileSize; xIncrement++) {
            for (var zIncrement = 0; zIncrement < cloudTileSize; zIncrement++) {
                var xVal = x + xIncrement;
                var zVal = z + zIncrement;
    ...
    {
        // Because clouds are 2D, if y is above or blow 0, return false.
        if (point.y != 0)
            return false;
    ...
    private GameObject CreateCloudTile(Mesh mesh, Vector3 position)
    {
        var newCloudTile = new GameObject
        {
            ...
            name = "Cloud " + position.x + ", " + position.z
        };
    ...
    {
        var a = value / (float) cloudTexWidth; // Get the position using cloudTexture width as units.
        a -= Mathf.FloorToInt(a); // Subtract whole numbers to get a 0-1 value representing position in cloud texture.
        var b = Mathf.FloorToInt((float) cloudTexWidth * a); // Multiply cloud texture width by a to get position in texture globally.
        return b;
    }
}
